...
def format_results_table(all_results: List[Dict]) -> str:
    ...
    lines.append("")
    lines.append(
        f"Policy gap (Current vs Full HR):          {(full_hr['observed_r0_zero_rate'] - current['observed_r0_zero_rate']) * 100:.2f} percentage points")
    lines.append(
        f"Policy gap (Current vs Theoretical):      {(theoretical['observed_r0_zero_rate'] - current['observed_r0_zero_rate']) * 100:.2f} percentage points")
    lines.append("")
    ...
    lines.append(
        f"Annual preventable infections (policy gap): {current_impact['annual_infections_with_policy'] - full_hr_impact['annual_infections_with_policy']:,}")
    lines.append("")
...
def generate_detailed_report(all_results: List[Dict]) -> str:
    ...
    for step in CASCADE_STEPS:
        ...
        report.append(
            f"  {step.name:<25}: P={step_prob:.3f}  (Cumulative: {cumulative:.4f}, Attrition: {attrition:.1f}%)")
