...
def simulate_individual(scenario: PolicyScenario, n_years: int = 5) -> Dict:
    ...
    if results["completed_cascade"]:
        ...
        survived = rng.random() < incarceration_survival
...
def main():
    ...
    output_path = '../pwid_simulation_results.json'
    csv_path = '../pwid_simulation_results.csv'
    try:
        ...
        with open(csv_path, 'w', newline='') as f:
            writer = csv.writer(f)
            header = [
                "Scenario", "Observed R0=0 Rate", "95% CI Lower", "95% CI Upper",
                "Cascade Completion", "Incarceration Survival", "Annual Infections",
                "5-Year Infections", "5-Year Cost Averted (Billions)"
            ]
            ...
            for r in json_results:
                writer.writerow([
                    r["scenario"],
                    f"{r['observed_r0_zero_rate']:.4f}",
                    f"{r['r0_zero_95ci'][0]:.4f}",
                    f"{r['r0_zero_95ci'][1]:.4f}",
                    f"{r['observed_cascade_completion_rate']:.4f}",
                    f"{r['incarceration_survival_probability']:.4f}",
                    r["impact"]["annual_infections_prevented"],
                    r["impact"]["five_year_infections_prevented"],
                    f"{r['impact']['five_year_cost_averted_billions']:.2f}"
                ])
