...
class ParameterWithUncertainty:
    ...
    def sample(self, n: int = 1) -> np.ndarray:
        ...
        if self.distribution == "uniform":
            ...
            samples = np.random.normal(self.point_estimate, std, n)
...
class EnhancedStochasticAvoidanceModel:
    ...
    def simulate_trajectory(
        ...
        """
        Simulate outbreak trajectories with uncertainty.
        """
        years = list(range(start_year, end_year + 1))
        n_years = len(years)
        
        # Storage for simulation results
        outbreak_years = []  # Year of first major outbreak in each simulation
        trajectories = {
            "meth_prevalence": np.zeros((n_simulations, n_years)),
            "network_density": np.zeros((n_simulations, n_years)),
            "outbreak_probability": np.zeros((n_simulations, n_years)),
            "cumulative_outbreak_prob": np.zeros((n_simulations, n_years)),
        }
        
        for sim in range(n_simulations):
            # Sample parameters if including uncertainty
            if include_uncertainty:
                meth_growth = self.params["meth_annual_growth_rate"].sample(1)[0]
                baseline_density = self.params["baseline_network_density"].sample(1)[0]
                meth_mult = self.params["meth_network_multiplier"].sample(1)[0]
                housing = self.params["housing_instability_rate"].sample(1)[0]
                incarc = self.params["incarceration_annual_rate"].sample(1)[0]
                ssp = self.params["ssp_coverage"].sample(1)[0]
                oat = self.params["oat_coverage"].sample(1)[0]
            else:
                meth_growth = self.params["meth_annual_growth_rate"].point_estimate
                baseline_density = self.params["baseline_network_density"].point_estimate
                meth_mult = self.params["meth_network_multiplier"].point_estimate
                housing = self.params["housing_instability_rate"].point_estimate
                incarc = self.params["incarceration_annual_rate"].point_estimate
                ssp = self.params["ssp_coverage"].point_estimate
                oat = self.params["oat_coverage"].point_estimate
            
            outbreak_occurred = False
            cumulative_no_outbreak = 1.0
            
            for i, year in enumerate(years):
                # Calculate meth prevalence
                years_from_baseline = year - 2018
                meth_prev = self.profile.baseline_prevalence_2018 * (
                    1 + meth_growth
                ) ** years_from_baseline
                meth_prev = min(meth_prev, 0.60)  # Saturation
                trajectories["meth_prevalence"][sim, i] = meth_prev
                
                # Calculate network density
                meth_effect = meth_prev * meth_mult * 0.5
                housing_effect = housing * 0.3
                incarc_effect = incarc * 0.2
                sex_work = meth_prev * 0.15
                
                density = baseline_density + meth_effect + housing_effect + incarc_effect + sex_work
                density = min(density, 1.0)
                trajectories["network_density"][sim, i] = density
                
                # Calculate outbreak probability
                p_outbreak = self.calculate_outbreak_probability(
                    density, ssp, oat, self.profile.hiv_prevalence_pwid
                )
                trajectories["outbreak_probability"][sim, i] = p_outbreak
                
                # Cumulative probability of no outbreak
                cumulative_no_outbreak *= (1 - p_outbreak)
                trajectories["cumulative_outbreak_prob"][sim, i] = 1 - cumulative_no_outbreak
                
                # Check if outbreak occurs this year (for time-to-outbreak)
                if not outbreak_occurred and np.random.random() < p_outbreak:
                    outbreak_years.append(year)
        ...
        if valid_outbreak_years:
            ...
            results["summary"].update({
                "median_years_to_outbreak": np.median(outbreak_times),
                "mean_years_to_outbreak": np.mean(outbreak_times),
                "std_years": np.std(outbreak_times),
                "p10_years": np.percentile(outbreak_times, 10),
                "p25_years": np.percentile(outbreak_times, 25),
                "p75_years": np.percentile(outbreak_times, 75),
                "p90_years": np.percentile(outbreak_times, 90),
                "p_outbreak_5yr": np.mean(outbreak_times <= 5),
                "p_outbreak_10yr": np.mean(outbreak_times <= 10),
            })
...
def plot_regional_meth_trajectories(save_path: str = None):
    ...
    # Panel A: All regions
    ax = axes[0]
    colors = plt.cm.Set2(np.linspace(0, 1, len(REGIONAL_PROFILES)))
    ...
    for i, (region, profile) in enumerate(REGIONAL_PROFILES.items()):
        if region != "national_average":
            ...
            ax.plot(trajectory["years"], trajectory["prevalences"],
                   label=profile.region, color=colors[i], linewidth=1.5)
    ...
    ax.plot(nat_trajectory["years"], nat_trajectory["prevalences"],
           'k--', linewidth=2, label="National Average")
    
    ax.axvline(x=2024, color='gray', linestyle=':', alpha=0.5, label='Current (2024)')
    ax.set_xlabel("Year")
    ax.set_ylabel("Meth-Opioid Co-Use Prevalence")
    ax.set_title("A. Regional Methamphetamine Trajectories", fontweight='bold')
    ...
    ax.fill_between(years, p5_density, p95_density, alpha=0.3, color='coral')
    ax.plot(years, mean_density, 'r-', linewidth=2, label='Network density')
    ...
    ax.set_title("B. Network Density Evolution\n(90% CI)", fontweight='bold')
...
def plot_outbreak_probability_forecast(results: Dict, save_path: str = None):
    ...
    ax.fill_between(years, p5_prob, p95_prob, alpha=0.3, color='crimson')
    ax.plot(years, mean_prob, 'darkred', linewidth=2)
    ...
    year_90 = years[np.argmax(mean_array >= 0.9)] if np.any(mean_array >= 0.9) else None
    
    if year_50:
        ax.axvline(x=year_50, color='orange', linestyle=':', alpha=0.7)
        ax.annotate(f'50% by {year_50}', xy=(year_50, 0.5), xytext=(year_50+2, 0.4),
                   fontsize=9, arrowprops=dict(arrowstyle='->', color='orange'))
    
    ax.set_xlabel("Year")
    ax.set_ylabel("Cumulative P(Major Outbreak)")
    ax.set_title("A. Outbreak Probability Forecast\n(90% CI)", fontweight='bold')
    ax.set_xlim(2024, 2040)
    ax.set_ylim(0, 1.0)
    ...
    if valid_years:
        ax.hist(valid_years, bins=range(0, 17), color='crimson', alpha=0.7,
               edgecolor='darkred', density=True)
        
        # Add summary statistics
        median = results["summary"].get("median_years_to_outbreak", 0)
        ax.axvline(x=median, color='black', linestyle='--', linewidth=2,
                  label=f'Median: {median:.1f} years')
        
        p5 = results["summary"].get("p10_years", 0)
        p95 = results["summary"].get("p90_years", 0)
        ax.axvspan(p5, p95, alpha=0.2, color='gray', label='80% CI')
    
    ax.set_xlabel("Years from 2024 to Major Outbreak")
    ax.set_ylabel("Density")
    ax.set_title("B. Time-to-Outbreak Distribution", fontweight='bold')
...
def plot_tornado_diagram(tornado_results: Dict, save_path: str = None):
    ...
    """
    fig, ax = plt.subplots(figsize=(DOUBLE_COL, 6))
    ...
    for i, p in enumerate(params):
        ...
        color_low = '#2E8B57' if low_diff < 0 else '#CD5C5C'
        color_high = '#CD5C5C' if high_diff > 0 else '#2E8B57'
        
        ax.barh(i, low_diff, height=0.6, left=baseline, color=color_low, alpha=0.8)
        ax.barh(i, high_diff, height=0.6, left=baseline, color=color_high, alpha=0.8)
    ...
    ax.set_yticklabels([p["parameter_label"] for p in params], fontsize=9)
    ax.set_xlabel(f'{tornado_results["outcome_metric"]} (baseline = {baseline:.2%})')
    ax.set_title("Tornado Diagram: Parameter Sensitivity\n(Red = increases outbreak risk, Green = decreases)",
                fontweight='bold')
    
    # Add legend
    from matplotlib.patches import Patch
    legend_elements = [
        Patch(facecolor='#CD5C5C', label='Increases outbreak risk'),
        Patch(facecolor='#2E8B57', label='Decreases outbreak risk')
    ]
...
def plot_scenario_comparison(scenario_results: Dict, save_path: str = None):
    ...
    colors = ['#CD5C5C' if p > 50 else '#DAA520' if p > 25 else '#2E8B57' 
              for p in p5yr]
    
    bars = ax.bar(range(len(scenarios)), p5yr, color=colors, alpha=0.8)
    
    ax.set_xticks(range(len(scenarios)))
    ax.set_xticklabels(scenarios, rotation=45, ha='right', fontsize=8)
    ax.set_ylabel("P(Outbreak within 5 years) %")
    ax.set_title("A. 5-Year Outbreak Risk by Scenario", fontweight='bold')
    ax.axhline(y=50, color='gray', linestyle='--', alpha=0.5)
    
    # Add value labels
    for bar, val in zip(bars, p5yr):
    ...
    colors = ['#2E8B57' if t > 10 else '#DAA520' if t > 5 else '#CD5C5C' 
              for t in median_times]
    
    bars = ax.bar(range(len(scenarios)), median_times, color=colors, alpha=0.8)
    
    ax.set_xticks(range(len(scenarios)))
    ax.set_xticklabels(scenarios, rotation=45, ha='right', fontsize=8)
    ax.set_ylabel("Median Years to Outbreak")
    ax.set_title("B. Time to Outbreak by Scenario", fontweight='bold')
    ax.axhline(y=5, color='red', linestyle='--', alpha=0.5, label='5-year horizon')
    ax.axhline(y=10, color='orange', linestyle='--', alpha=0.5, label='10-year horizon')
    
    # Add value labels
    for bar, val in zip(bars, median_times):
...
def main():
    ...
    with open(f"{output_dir}/stochastic_avoidance_sensitivity_results.json", 'w') as f:
        json.dump(all_results, f, indent=2, default=str)
    
    print(f"\n   Results saved to {output_dir}/stochastic_avoidance_sensitivity_results.json")
