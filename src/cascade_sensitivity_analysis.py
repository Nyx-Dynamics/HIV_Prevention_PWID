...
class CascadeSensitivityAnalyzer:
    ...
    def run_probabilistic_sensitivity(
        ...
        current_scenario = PolicyScenario("Current Policy", "Baseline conditions")
    ...
    def barrier_removal_analysis(self, n_individuals: int = 100000) -> Dict:
        ...
        baseline = model.run_simulation(PolicyScenario("Current Policy", "Baseline conditions"), n_individuals=n_individuals)
        results["baseline"] = {
        ...
        barrier_scenarios = [
            ("no_criminalization", PolicyScenario("No Criminalization", "Decriminalization scenario", decriminalization=True, incarceration_modifier=0.3)),
            ("no_stigma", PolicyScenario("No Stigma", "Full stigma reduction", stigma_reduction=1.0, bias_training=True)),
            ("low_barrier_access", PolicyScenario("Low Barrier Access", "Low barrier harm reduction", ssp_integrated_delivery=True, peer_navigation=True, low_barrier_access=True)),
            ("full_inclusion", PolicyScenario("Full Research Inclusion", "PWID trial inclusion", pwid_trial_inclusion=True, algorithmic_debiasing=True)),
            ("all_removed", PolicyScenario("All Barriers Removed", "All structural barriers removed", decriminalization=True, incarceration_modifier=0.0,
                                           stigma_reduction=1.0, bias_training=True, ssp_integrated_delivery=True,
                                           peer_navigation=True, low_barrier_access=True, pwid_trial_inclusion=True, algorithmic_debiasing=True)),
        ]
