...
# BMC Public Health dimensions (mm converted to inches)
MM_TO_INCH = 1 / 25.4
WIDTH_SINGLE = 84 * MM_TO_INCH
WIDTH_DOUBLE = 174 * MM_TO_INCH

# BMC Public Health publication quality settings
plt.rcParams.update({
...
def plot_regional_contextual_failure_driver_trajectories(save_path: str = None):
    ...
    plt.tight_layout()
    output_dir = save_path if save_path else os.path.join(os.path.dirname(__file__), "..", "data", "figures")
...
def main():
    ...
    try:
        json_path = f"{data_dir}/stochastic_avoidance_sensitivity_results.json"
        with open(json_path, 'w') as f:
            json.dump(all_results, f, indent=2, default=str, allow_nan=False)
        print(f"\n   Results saved to {json_path}")

        # Save to CSV
        csv_path = f"{data_dir}/stochastic_avoidance_sensitivity_results.csv"
        ...
        # Save to Excel
        xlsx_path = f"{data_dir}/stochastic_avoidance_sensitivity_results.xlsx"
        try:
