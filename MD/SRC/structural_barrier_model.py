#!/usr/bin/env python3
"""
Structural Barrier Model: HIV Prevention Barrier Modeling for PWID
==================================================================
...
THEORETICAL FOUNDATION:
    This model builds on the Prevention Theorem framework which establishes that
    HIV prevention (R₀ = 0) is biologically constrained to a narrow window before
    proviral integration (~24h for parenteral, ~72h for mucosal exposure).

    The Prevention Theorem answers: "When must prevention occur?"
    This model answers: "Why doesn't prevention occur in time?"

    By modeling the structural barriers that prevent timely intervention, this
    analysis explains why theoretical biological prevention windows are not
    achieved in practice for PWID populations.

PRIMARY HYPOTHESES:
    1. Nested barriers create conditions where R(0)=0 is mathematically unachievable
...
THREE-LAYER BARRIER FRAMEWORK:
    Layer 1: Pathogen Biology (irreversible R(0)>0 within hours)
        - Links to Prevention Theorem: proviral integration establishes R(0)>0
        - Parenteral exposure compresses prevention window vs mucosal

    Layer 2: HIV Testing Failures (acute infection detection gaps)
        - Window period limitations
        - Testing frequency insufficiency

    Layer 3: Architectural Failures
...
See also:
    - prevention_theorem_figures.py: Theoretical framework visualizations
    - pep_mucosal.py: PEP timing analysis connecting theory to this barrier model
...
class ArchitecturalSubtype(Enum):
    """Subtypes of architectural failures"""
    POLICY = "policy"
    STIGMA = "stigma"
...
class ArchitecturalBarrierParams:
    """Layer 3: Architectural failure parameters"""
    ...
    # 3e. Machine learning / WMD barriers
...
def create_pwid_cascade() -> List[CascadeStep]:
    ...
    - Architectural failures (5 subtypes)
    """
...
class ManufacturedDeathModel:
    ...
    def __init__(
        self,
        pathogen_params: Optional[PathogenBiologyParams] = None,
        testing_params: Optional[HIVTestingParams] = None,
        architectural_params: Optional[ArchitecturalBarrierParams] = None,
        stochastic_params: Optional[StochasticAvoidanceParams] = None
    ):
...
class StochasticAvoidanceModel:
    ...
    def __init__(self, params: Optional[StochasticAvoidanceParams] = None):
    ...
    def simulate_time_to_outbreak(
        ...
        for _ in range(n_simulations):
            ...
            for year in range(2024, 2024 + max_years):
                ...
                if random.random() < p_outbreak:
                    outbreak_years_list.append(year - 2024)
        ...
        # Calculate statistics
        if outbreak_years_list:
            outbreak_years = np.array(outbreak_years_list)
            results: Dict[str, Any] = {
...
def main():
    ...
    print()
    print("Architectural Failure Subtypes:")
    ...
    output_path = "structural_barrier_results.json"
    csv_path = "structural_barrier_results.csv"
    try:
        ...
        with open(csv_path, 'w', newline='') as f:
            ...
            # MSM comparison
            writer.writerow([])
            writer.writerow(["MSM Comparison", "", "", "", f"{msm_results['p_r0_zero']:.6f}", f"{msm_results['cascade_completion']:.6f}", "N/A", "N/A"])
        ...
        # Save to Excel
        xlsx_path = "structural_barrier_results.xlsx"
        try:
            with pd.ExcelWriter(xlsx_path, engine='openpyxl') as writer:
                ...
                df_scenarios = pd.DataFrame([
                    {
                        "Scenario": r["scenario"],
                        "Achieved R0=0": r["achieved_r0_zero"],
                        "Completed Cascade": r["completed_cascade"],
                        "Incarceration Disrupted": r["incarceration_disrupted"],
                        "Observed R0=0 Rate": r["observed_r0_zero_rate"],
                        "Observed Cascade Rate": r["observed_cascade_completion_rate"],
                        "95% CI Lower": r["r0_zero_95ci"][0],
                        "95% CI Upper": r["r0_zero_95ci"][1]
                    } for r in all_results
                ])
                ...
                # MSM Comparison
                df_msm = pd.DataFrame([{
                    "Metric": "MSM Comparison",
                    "P(R0=0)": msm_results['p_r0_zero'],
                    "Cascade Completion": msm_results['cascade_completion'],
                    "SNR": msm_results['snr'],
                    "In Training Set": msm_results['in_training_set']
                }])
                df_msm.to_excel(writer, sheet_name="MSM Comparison", index=False)

                # Barrier Decomposition (Current Policy)
                current = all_results[0]
                if "three_layer_decomposition" in current:
                    df_layers = pd.DataFrame([
                        {"Layer": layer.replace('_', ' ').title(), "Percentage (%)": pct}
                        for layer, pct in current["three_layer_decomposition"].items()
                    ])
                    df_layers.to_excel(writer, sheet_name="Barrier Decomposition", index=False)

                if "barrier_decomposition_pct" in current:
                    df_subtypes = pd.DataFrame([
                        {"Subtype": barrier.replace('_', ' ').title(), "Percentage (%)": current["barrier_decomposition_pct"].get(barrier, 0)}
                        for barrier in ["policy", "stigma", "infrastructure", "research_exclusion", "machine_learning"]
                    ])
                    df_subtypes.to_excel(writer, sheet_name="Architectural Subtypes", index=False)
    ...
    print("=" * 80)
    print()
    print("1. MATHEMATICAL IMPOSSIBILITY OF R(0)=0")
