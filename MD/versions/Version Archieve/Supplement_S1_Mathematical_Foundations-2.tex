...
% Supporting: Manufactured Death: Mathematical Modeling of HIV Prevention 
% Impossibility Among People Who Inject Drugs
% December 2024
...
\textit{Manufactured Death: Mathematical Modeling of HIV Prevention\\Impossibility Among People Who Inject Drugs}}
...
For $\varepsilon_{\text{drug}} = 0.999$, achieving $R_e < 0.01$ requires:
...
\textbf{Definition 3.1} (Prevention Cascade). The cascade is a multiplicative sequence:
...
\section{Mathematical Definition of Manufactured Death}

\textbf{Definition 7.1} (Manufactured Death). A death is ``manufactured'' if and only if:

\begin{enumerate}
    \item An intervention exists with efficacy $\varepsilon > \varepsilon^*$ (elimination threshold)
    \item Cascade completion $\pi < \pi^*$ (below implementation threshold)
    \item $\pi < \pi^*$ results from policy decisions, not biological/behavioral limits
\end{enumerate}

\textbf{Theorem 7.1}. HIV deaths among PWID meet the definition of manufactured death.

\textbf{Proof.}

\textit{Condition 1:} LAI-PrEP efficacy $\varepsilon = 0.999 > 0.96 = \varepsilon^*$. $\checkmark$

\textit{Condition 2:} PWID cascade completion $\pi = 0.000007 < 0.996 = \pi^*$. $\checkmark$

\textit{Condition 3:} Under comprehensive reform:
\begin{equation}
\pi_{\text{reform}} = 0.199 \gg 0.000007 = \pi_{\text{current}}
\end{equation}

The 28,000-fold improvement under policy change demonstrates current $\pi$ reflects policy, not population characteristics. Furthermore, HCV trials demonstrate PWID achieve equivalent outcomes (94\% vs 97\%) when included.

Therefore, $\pi < \pi^*$ is policy-constructed, not biologically determined. $\checkmark$

All conditions satisfied. HIV deaths among PWID are manufactured. $\square$

\section{Intervention Mathematics}

\textbf{Proposition 8.1} (Removing Root Barrier). If the root barrier $B_0$ is neutralized (e.g., regulatory requirement for PWID inclusion):
...
\textbf{Theorem 8.2} (Futility of Downstream-Only Intervention). Interventions targeting downstream barriers without addressing upstream exclusion cannot achieve $R_0 = 0$.
...
\textit{This supplement provides the mathematical foundations for the manuscript ``Manufactured Death: Mathematical Modeling of HIV Prevention Impossibility Among People Who Inject Drugs.'' All theorems and proofs are original unless otherwise cited.}
